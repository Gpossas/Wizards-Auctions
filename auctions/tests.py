...
#         """
#         c = Client()
#         response = json.loads(c.get(reverse('categories')))
...
#         c = Client()
#         response = json.loads(c.get(reverse('category', args=[pk])))
#         self.assertEqual(response.status_code, 200)

#     def test_invalid_category_raises_http404(self):
#         c = Client()
#         invalid_index = 0
#         response = json.loads(c.get(reverse('category', args=[invalid_index])))
...
#         """
#         c = Client()
#         response = json.loads(c.get(reverse('category', args=[1])))
#         self.assertEqual(response.status_code, 200)
...
#         response = json.loads(self.client.get(reverse('watchlist')))
#         self.assertEqual(response.status_code, 200)
...
#         """
#         response = json.loads(json.loads(self.client.get(reverse('watchlist'))))
#         self.assertEqual(response.status_code, 302)
#         self.assertEqual(response.url, f"{reverse('login')}?next={reverse('watchlist')}")
#         response = json.loads(self.client.post(reverse('watchlist_change_state', args=[self.potion.id])))
#         self.assertEqual(response.status_code, 302)
#         self.assertEqual(response.url, f"{reverse('login')}?next={reverse('watchlist_change_state', args=[self.potion.id])}")
    
#     def test_valid_watchlist_change_state_add(self):
#         try:
            
#             client = Client(enforce_csrf_checks=True)
#             client.login(username="Alvo Dumbledore", password="123")
#             print('#)(!)#', )
#             client.get(reverse('watchlist_change_state', args=[self.potion.id]))
#             print('###$%###', )
#             client.cookies['csrftoken']
#             print('#56789#')
#             csrftoken = client.cookies['csrftoken']
#             response = requests.post(
#                 f"http://127.0.0.1:8000/{reverse('watchlist_change_state', args=[self.potion.id])}",
#                 headers = { 'X-CSRFToken': csrftoken, 'Content-Type': 'application/json'},
#                 data=json.dumps({'new_state': 'in_watchlist'})
#             )
#             print('##########123##############',response,'#',response.content,'#########################')
#             response_2 = self.client.post(
#                 reverse('watchlist_change_state', args=[self.potion.id]),
#                 headers = { 'X-CSRFToken': 's', 'Content-Type': 'application/json'},
#                 data=json.dumps({'new_state': 'in_watchlist'})
#             )
#             print('##########456##############',response_2,'#',response.content,'#########################')
#             self.assertEqual(response.status_code, 302)
#         except Exception as e:
#             print('##########error##############',e,'#########################')
...
#     def test_invalid_watchlist_change_state_add(self):
#         self.client.force_login(self.user)
#         response = self.client.post(reverse('watchlist_change_state', args=[0]), {'watchlist': ''})
        
#         self.assertEqual(response.status_code, 404)

#     def test_watchlist_change_state_deletion(self):
#         Watchlist.objects.create(user=self.user, listing=self.potion)

#         self.client.force_login(self.user)
#         response = json.loads(self.client.post(reverse('watchlist_change_state', args=[self.potion.id]), {'watchlist': 'True'}))
#         self.assertEqual(response.status_code, 302)
...
#     def test_fail_watchlist_change_state_deletion(self):
#         self.client.force_login(self.user)
#         response = json.loads(self.client.post(reverse('watchlist_change_state', args=[self.potion.id]), {'watchlist': 'True'}))
#         self.assertEqual(response.status_code, 302)
...
#         response = json.loads(client.post(reverse('place_bid'), {'bid': self.max_bid.price + 1}))
...
#         client = Client()
#         client.force_login(self.user)
#         response = json.loads(client.post(reverse('place_bid'), {'bid': 'venice'}))
...
#         self.client.force_login(self.user)
#         response = json.loads(self.client.post(reverse('comments', args=[self.listing.id]), {'comment': 'hello world'}))

#         self.assertEqual(response.status_code, 200)
#         self.assertEqual(response['text'], 'hello world')
#         self.assertEqual(response['user'], 'Alvo Dumbledore')
#         self.assertEqual(response.url, f"{reverse('listing_page', args=[self.listing.id])}")

#     def test_blank_comments(self):
#         self.client.force_login(self.user)
#         response = json.loads(self.client.post(reverse('comments', args=[self.listing.id]), {'comment': ' '}))
...
#         client = Client()
#         response = json.loads(client.post(reverse('listing_state', args=[self.listing.id]), {'active': ''}))
#         self.assertEqual(response.status_code, 302)
...
#         """
#         response = json.loads(self.client.post(reverse('listing_state', args=[0])))
#         self.assertEqual(response.status_code, 404)
...
#         response = json.loads(self.client.post(reverse('listing_state', args=[listing.id]), {'active': 'True'}))
#         self.assertEqual(response.status_code, 302)
...
#         response = json.loads(self.client.post(reverse('listing_state', args=[listing.id]), {'active': ''}))
#         self.assertEqual(response.status_code, 302)
#         state_before = listing.active
#         state_after = Listing.objects.get(pk=listing.id).active
#         self.assertNotEqual(state_before, state_after)
...
#         """
#         client = Client()
#         response = json.loads(client.get(reverse('create_listing')))
#         self.assertEqual(response.status_code, 302)
...
#         """
#         response = json.loads(self.client.get(reverse('create_listing')))
...
#         }
#         response = json.loads(self.client.post(reverse('create_listing'), add_listing))
#         self.assertEqual(response.status_code, 302)
...
#         """
#         client = Client()
#         response = json.loads(client.get(reverse('listing_page', args=[self.listing.id])))
#         self.assertEqual(response.status_code, 200)
...
#         response = json.loads(self.client.get(reverse('listing_page', args=[max_id+1])))
#         self.assertEqual(response.status_code, 404)
    
#     def test_listing_page_not_in_watchlist(self):
#         response = json.loads(self.client.get(reverse('listing_page', args=[self.listing.id])))
#         self.assertEqual(response.status_code, 200)
...
#         response = json.loads(self.client.get(reverse('listing_page', args=[ankh_shield_listing.id])))
...
#         response = json.loads(self.client.get(reverse('index')))
#         self.assertEqual(response.status_code, 200)
